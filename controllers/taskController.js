...
exports.postNewTask = async (req, res) => {
  try {
    ...
    const queryClient = "SELECT adminId From users Where id = ?";
    const clientId = await executeQuery(queryClient, [userId])
    ...
    if (clientId[0].adminId !== id) {
      return res.status(401).json({ message: "You are not admin for this user" })
    }
    const query =
...
exports.updateTaskByID = async (req, res) => {
  try {
    ...
    if (userRole === "admin") {
      ...
      const queryTaskClient = "SELECT userId From tasks Where id = ?";
      const TaskclientId = await executeQuery(queryTaskClient, [id]);

      if (TaskclientId.length === 0) {
        return res.status(500).json({ message: "Unexpected error occurred" });
      }
      const queryClient = "SELECT adminId From users Where id = ?";
      const clientId = await executeQuery(queryClient, [TaskclientId[0].userId]);
      ...
      if (clientId[0].adminId !== req.RealId) {
        return res.status(401).json({ message: "You are not admin for this user" })
      }
